...
import { parseGarminCsv } from '@/lib/parsers/garmin';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const garminData = parseGarminCsv(csvContent, profile.id);

    if (garminData.length > 0) {
      const { error } = await supabase
        .from('garmin_daily')
        .upsert(
          garminData.map((g) => ({ ...g, upload_id })),
    ...
    return NextResponse.json({
      success: true,
      days_imported: garminData.length,
    });
  } catch (error) {
    await supabase
      .from('uploads')
